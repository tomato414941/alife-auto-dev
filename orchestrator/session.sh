...
# Helper: run prompt through randomly chosen engine
# Returns engine name via LAST_ENGINE variable for logging
run_agent() {
  local prompt="$1"
  local workdir="$2"
  local log_out="$3"
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Session Planner finished (engine=$LAST_ENGINE, exit=$PLANNER_EXIT)"
...
if true; then
  ...
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Critic finished (engine=$LAST_ENGINE, exit=$CRITIC_EXIT)"
